...
list<-c("ggplot2", "data.table", "reshape2", "arm","car", "DMwR", "Hmisc",
        ...
        "sf", "glmmTMB", "DHARMa", "sjPlot", "ggpubr", "ggfortify", "scales", "kableExtra", "vegan",    
        "viridis",  "archetypes", "rnaturalearth")
...
a <- unique(mydata[, c("SciName","PC1", "PC2", "PC3")])
#saveRDS(a,"SI_dataset.rds")
pca_traits <- readRDS("Data_pca_traits.rds")
...
```{r}
tab <- function(sum.df, caption = "Table S2.1. Model outputs for communities at the level of haul") { sum.df <- as_tibble(sum.df[, c("term", "estimate", "std.error", "statistic", "p.value")]) 
sum.df %>% 
...
```{r}
pc <- prcomp(pca_traits[,c("tl", "offspring.size","age.maturity", "fecundity", "growth.coefficient", "length.max", "age.max")])
print(pc)
summary(pc)
```
...
pca_traits$groups <- 1
pca_traits$groups[pca_traits$taxon %in% c("Lamna nasus", #"Polymetme thaeocoryla", 
                        "Spinachia spinachia",
                        #"Sprattus sprattus",
  "Gadus morhua")] <- 0
pca_traits$groups <- factor(pca_traits$groups)
autoplot(pc, data = pca_traits, #colour = "grey", 
...
```{r}
sum(h$Total_DensAbund_N_Sqkm[h$SciName %in% x$taxon], na.rm = T)
sum(h$Total_DensAbund_N_Sqkm[h$SciName %nin% x$taxon], na.rm = T)/sum(h$Total_DensAbund_N_Sqkm, na.rm = T)
...
```{r}
s <- unique(cwm_main[, c("HaulID", "res5")])
x5 <- s %>% group_by(res5) %>% count()

s <- unique(cwm_main[, c("HaulID", "res10")])
x10 <- s %>% group_by(res10) %>% count()

s <- unique(cwm_main[, c("HaulID", "res20")])
x20 <- s %>% group_by(res20) %>% count()

s <- unique(cwm_main[, c("HaulID", "res50")])
x50 <- s %>% group_by(res50) %>% count()
```

```{r}
par(mfrow = c(4,1))
hist((x5$n), breaks = 100)
hist((x10$n), breaks = 100)
hist((x20$n), breaks = 100)
hist((x50$n), breaks = 100)
...
sd(x5$n)/sqrt(length((x5$n)))
sd(x10$n)/sqrt(length((x10$n)))
sd(x20$n)/sqrt(length((x20$n)))
sd(x50$n)/sqrt(length((x50$n)))


hist(log(x5$n), breaks = 100)
hist(log(x10$n), breaks = 100)
hist(log(x20$n), breaks = 100)
hist(log(x50$n), breaks = 100)
```
