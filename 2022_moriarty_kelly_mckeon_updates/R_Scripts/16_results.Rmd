...
```
```{r}
setwd("~/Library/CloudStorage/OneDrive-Personal/PhD/Fishies/fishies/2022_moriarty_kelly_mckeon_updates/cwm_30")
pc1_null30 <- readRDS("PC1_null.rds")
pc2_null30 <- readRDS("PC2_null.rds")
pc3_null30 <- readRDS("PC3_null.rds")
pc1_30 <- readRDS("PC1_2way.rds")
pc2_30 <- readRDS("PC2_2way.rds")
pc3_30 <- readRDS("PC3_2way.rds")
```
```{r}
setwd("~/Library/CloudStorage/OneDrive-Personal/PhD/Fishies/fishies/2022_moriarty_kelly_mckeon_updates/cwm_30")
pc1_null40 <- readRDS("PC1_null.rds")
pc2_null40 <- readRDS("PC2_null.rds")
pc3_null40 <- readRDS("PC3_null.rds")
pc1_40 <- readRDS("PC1_2way.rds")
pc2_40 <- readRDS("PC2_2way.rds")
pc3_40 <- readRDS("PC3_2way.rds")
```
...
g30_pc1 <- broom.mixed::tidy(pc1_30, effects = "fixed", conf.int = TRUE, conf.level = 0.95)
g30_pc2 <- broom.mixed::tidy(pc2_30, effects = "fixed", conf.int = TRUE, conf.level = 0.95)
g30_pc3 <- broom.mixed::tidy(pc3_30, effects = "fixed", conf.int = TRUE, conf.level = 0.95)

g40_pc1 <- broom.mixed::tidy(pc1_40, effects = "fixed", conf.int = TRUE, conf.level = 0.95)
g40_pc2 <- broom.mixed::tidy(pc2_40, effects = "fixed", conf.int = TRUE, conf.level = 0.95)
g40_pc3 <- broom.mixed::tidy(pc3_40, effects = "fixed", conf.int = TRUE, conf.level = 0.95)
...
g1 <- rbind(g_pc1 , g5_pc1, g10_pc1, g20_pc1,  g50_pc1) #g30_pc1, g40_pc1,
g2 <- rbind(g_pc2 , g5_pc2, g10_pc2, g20_pc2, g50_pc2) #g30_pc2, g40_pc2, 
g3 <- rbind(g_pc3 , g5_pc3, g10_pc3, g20_pc3,  g50_pc3) # g30_pc3, g40_pc3,

g1$res <- factor(g1$res,levels = c("1", "5", "10", "20",  "50")) #"30","40",
g2$res <- factor(g2$res,levels = c("1", "5", "10", "20",  "50")) #"30","40",
g3$res <- factor(g3$res,levels = c("1", "5", "10", "20",  "50")) #"30","40",
...
x <- 2
b1 <- ggplot(g1[g1$res == x,],  aes(term, estimate, group = res)) +
  geom_hline(yintercept= 0, alpha = 0.8) +
           geom_rect(data = g1[g1$rep != "white",],
             aes(NULL, NULL, xmin = cat - 0.5, xmax = cat + 0.5, ymin = -Inf, ymax = Inf, alpha = 0.01),
            colour = NA, alpha = 0.01) +
  geom_pointrange(data = g1[g1$res == x,], shape = 1,
                    aes(term, estimate, colour = main1,
                        ymin= conf.low, ymax= conf.high),
                    position = position_dodge(0.9), size = 1.5) +
     geom_point(data = g1[g1$res == x,],
                    aes(term, estimate, colour = main2, size = res),
                    position = position_dodge(0.9)) +
  labs(colour = "Model terms", y = "Effect size PC1", colour = "resolution") + 
  scale_color_manual(values = c("SNSP" = "#414487FF",
...
b2 <- ggplot(g2[g2$res == x,],  aes(term, estimate, group = res)) +
  geom_hline(yintercept= 0, alpha = 0.8) +
           geom_rect(data = g2[g2$rep != "white",],
             aes(NULL, NULL, xmin = cat - 0.5, xmax = cat + 0.5, ymin = -Inf, ymax = Inf, alpha = 0.01),
            colour = NA, alpha = 0.01) +
  geom_pointrange(data = g2[g2$res == x,], shape = 1,
                    aes(term, estimate, colour = main1,
                        ymin= conf.low, ymax= conf.high),
                    position = position_dodge(0.9), size = 1.5) +
     geom_point(data = g2[g2$res == x,],
                    aes(term, estimate, colour = main2, size = res),
                    position = position_dodge(0.9)) +
  labs(colour = "Model terms", y = "Effect size PC2", colour = "resolution") + 
  scale_color_manual(values = c("SNSP" = "#414487FF",
...
b3 <- ggplot(g3[g3$res == x,],  aes(term, estimate, group = res)) +
  geom_hline(yintercept= 0, alpha = 0.8) +
           geom_rect(data = g3[g3$rep != "white",],
             aes(NULL, NULL, xmin = cat - 0.5, xmax = cat + 0.5, ymin = -Inf, ymax = Inf, alpha = 0.01),
            colour = NA, alpha = 0.01) +
  geom_pointrange(data = g3[g3$res == x,], shape = 1,
                    aes(term, estimate, colour = main1,
                        ymin= conf.low, ymax= conf.high),
                    position = position_dodge(0.9), size = 1.5) +
     geom_point(data = g3[g3$res == x,],
                    aes(term, estimate, colour = main2, size = res),
                    position = position_dodge(0.9)) +
  labs(colour = "Model terms", y = "Effect size PC3", colour = "resolution") + 
  scale_color_manual(values = c("SNSP" = "#414487FF",
...
## magnitude of significant effects in coarsest res models
```{r}
m1 <- g1[, c("term", "estimate", "std.error", "statistic", "p.value", "res")]
sum.df <- as_tibble(m1[m1$res == 5,])
x1 <- sum.df[sum.df$p.value <= 0.05,]
x1$e2 <- sqrt(x1$estimate^2)

m1 <- g2[, c("term", "estimate", "std.error", "statistic", "p.value", "res")]
sum.df <- as_tibble(m1[m1$res == 5,])
x2 <- sum.df[sum.df$p.value <= 0.05,]
x2$e2 <- sqrt(x2$estimate^2)

m1 <- g3[, c("term", "estimate", "std.error", "statistic", "p.value", "res")]
sum.df <- as_tibble(m1[m1$res == 5,])
x3 <- sum.df[sum.df$p.value <= 0.05,]
x3$e2 <- sqrt(x3$estimate^2)
```

## magnitude of significant effects in finest res models
```{r}
m1 <- g1[, c("term", "estimate", "std.error", "statistic", "p.value", "res")]
sum.df <- as_tibble(m1[m1$res == 0.1,])
x1 <- sum.df[sum.df$p.value <= 0.05,]
...
plot(tmp_raster, xaxs="i",yaxs="i", col = pal(50), legend=FALSE, ext=extent(e))
...
sum.df %>% 
  kable(booktabs = T, caption = "Table C.2. Consistent core areas of 50 species after Probst et al., 2021") %>%
  kable_styling(bootstrap_options = "striped", full_width = T) %>%
  kable_styling(font_size = 11)
```
...
## make a table of species' PC values 
```{r}
abundance <- as_tibble(unique(mydata[, c("SciName", "PC1", "PC2", "PC3")]))
abundance$PC1 <- round(abundance$PC1, 4)
abundance$PC2 <- round(abundance$PC2, 4)
abundance$PC3 <- round(abundance$PC3, 4)
abundance <- abundance[order(abundance$SciName),] 
names(abundance) <- c("taxon", "PC1", "PC2", "PC3")
#write.csv(abundance, "McKeon_fishing_lifehistory_scale_SI_dataset.csv")
```


## look at which species were lost as they had no trait data
```{r}
x <- drop_na(as.data.frame(unique(h[h$SciName %nin% abundance$taxon, c("SciName")])))
names(x) <- "taxon"
library(worrms)
worms_taxonomy <- wm_records_taxamatch(x$taxon)

c <- as.data.frame(t(1:length((worms_taxonomy[[1]]))))
names(c) <- names(worms_taxonomy[[1]])
c <- c[0,]
for(i in 1:length(worms_taxonomy)){
  c <- rbind(c, worms_taxonomy[[i]])
}
missed_taxa <- c

worms_taxonomy <- wm_records_taxamatch(x$taxon[x$taxon %nin% missed_taxa$scientificname])
c <- as.data.frame(t(1:length((worms_taxonomy[[1]]))))
names(c) <- names(worms_taxonomy[[1]])
c <- c[0,]
for(i in 1:length(worms_taxonomy)){
...
missed_taxa <- rbind(missed_taxa, c)

x <- missed_taxa %>% group_by(rank) %>% count()

worms_taxonomy1 <- wm_records_taxamatch(abundance$taxon[1:50])
worms_taxonomy2 <- wm_records_taxamatch(abundance$taxon[51:100])
worms_taxonomy3 <- wm_records_taxamatch(abundance$taxon[101:150])
worms_taxonomy4 <- wm_records_taxamatch(abundance$taxon[151:200])
worms_taxonomy5 <- wm_records_taxamatch(abundance$taxon[200:229])

worms_taxonomy <- c(worms_taxonomy1, worms_taxonomy2, worms_taxonomy3, worms_taxonomy4, worms_taxonomy5)

d <- as.data.frame(t(1:length((worms_taxonomy[[1]]))))
names(d) <- names(worms_taxonomy[[1]])
d <- d[0,]
for(i in 1:length(worms_taxonomy)){
  d <- rbind(d, worms_taxonomy[[i]])
}

taxa <- d

x <- taxa %>% group_by(valid_name) %>% count()

# saveRDS(taxa, "Data_taxa_analysed.rds")
# saveRDS(missed_taxa, "Data_taxa_not_analysed.rds")
```
