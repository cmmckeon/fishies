...
list<-c("ggplot2", "data.table", "reshape2", "arm","car", "DMwR", "Hmisc",
        ...
        "sf", "glmmTMB", "DHARMa", "sjPlot", "ggpubr", "ggfortify", "scales")
...
```{r}
setwd("~/Library/CloudStorage/OneDrive-Personal/PhD/Fishies/fishies/2022_moriarty_kelly_mckeon_updates/cwm_models")
...
g_pc1$main1 <- factor(g_pc1$main1, levels = c("Depth", "SNSP", "SNWI", "sst_var",  "fp","Quarter","Year"))
g_pc1$term <- factor(g_pc1$term, levels = c("Depth", "SNSP", "SNWI", "sst_var", "fp", "Depth:fp", "SNSP:fp", "SNWI:fp",  "sst_var:fp",
...
g_pc2$main1 <- factor(g_pc2$main1, levels = c("Depth", "SNSP", "SNWI", "sst_var",  "fp","Quarter","Year"))
g_pc2$term <- factor(g_pc2$term, levels = c("Depth", "SNSP", "SNWI", "sst_var", "fp", "Depth:fp", "SNSP:fp", "SNWI:fp",  "sst_var:fp",
                                             "SNSP:SNWI", "SNSP:sst_var", "SNWI:sst_var", "SNSP:Depth",  "SNWI:Depth", "sst_var:Depth", 
                                              "Quarter", "Year"))
...
g_pc3$main1 <- factor(g_pc3$main1, levels = c("Depth", "SNSP", "SNWI", "sst_var",  "fp","Quarter","Year"))
g_pc3$term <- factor(g_pc3$term, levels = c("Depth", "SNSP", "SNWI", "sst_var", "fp", "Depth:fp", "SNSP:fp", "SNWI:fp",  "sst_var:fp",
                                             "SNSP:SNWI", "SNSP:sst_var", "SNWI:sst_var", "SNSP:Depth",  "SNWI:Depth", "sst_var:Depth", 
                                              "Quarter", "Year"))
## create column for grey bars
g_pc1$rep <- as.character("white")
g_pc1$rep[g_pc1$main %in% c("fp", "SNSP:fp", "SNWI:fp", "sst_var:fp","Depth:fp")] <- "grey"
g_pc1$cat <- as.numeric(g_pc1$term)

g_pc2$rep <- as.character("white")
g_pc2$rep[g_pc2$main %in% c("fp", "SNSP:fp", "SNWI:fp", "sst_var:fp","Depth:fp")] <- "grey"
g_pc2$cat <- as.numeric(g_pc2$term)

g_pc3$rep <- as.character("white")
g_pc3$rep[g_pc3$main %in% c("fp", "SNSP:fp", "SNWI:fp", "sst_var:fp","Depth:fp")] <- "grey"
...
g1$main1 <- factor(g1$main1, levels = c("Depth", "SNSP", "SNWI", "sst_var",  "fp","Quarter","Year"))
g1$term <- factor(g1$term, levels = c("Depth", "SNSP", "SNWI", "sst_var", "fp", "Depth:fp", "SNSP:fp", "SNWI:fp",  "sst_var:fp",
                                             "SNSP:SNWI", "SNSP:sst_var", "SNWI:sst_var", "SNSP:Depth",  "SNWI:Depth", "sst_var:Depth", 
                                              "Quarter", "Year"))
...
g2$main1 <- factor(g2$main1, levels = c("Depth", "SNSP", "SNWI", "sst_var",  "fp","Quarter","Year"))
g2$term <- factor(g2$term, levels = c("Depth", "SNSP", "SNWI", "sst_var", "fp", "Depth:fp", "SNSP:fp", "SNWI:fp",  "sst_var:fp",
                                             "SNSP:SNWI", "SNSP:sst_var", "SNWI:sst_var", "SNSP:Depth",  "SNWI:Depth", "sst_var:Depth", 
                                              "Quarter", "Year"))
...
g3$main1 <- factor(g3$main1, levels = c("Depth", "SNSP", "SNWI", "sst_var",  "fp","Quarter","Year"))
g3$term <- factor(g3$term, levels = c("Depth", "SNSP", "SNWI", "sst_var", "fp", "Depth:fp", "SNSP:fp", "SNWI:fp",  "sst_var:fp",
                                             "SNSP:SNWI", "SNSP:sst_var", "SNWI:sst_var", "SNSP:Depth",  "SNWI:Depth", "sst_var:Depth", 
                                              "Quarter", "Year"))
## create column for grey bars
g1$rep <- as.character("white")
g1$rep[g1$main %in% c("fp", "SNSP:fp", "SNWI:fp", "sst_var:fp","Depth:fp")] <- "grey"
g1$cat <- as.numeric(g1$term)

g2$rep <- as.character("white")
g2$rep[g2$main %in% c("fp", "SNSP:fp", "SNWI:fp", "sst_var:fp","Depth:fp")] <- "grey"
g2$cat <- as.numeric(g2$term)

g3$rep <- as.character("white")
g3$rep[g3$main %in% c("fp", "SNSP:fp", "SNWI:fp", "sst_var:fp","Depth:fp")] <- "grey"
...
# ggplot(g_pc1,  aes(term, estimate)) +
#   
#   geom_pointrange(data = g_pc1, shape = 1,
#                     aes(term, estimate, colour = main1, 
#                         ymin= conf.low, ymax= conf.high),
...
#     geom_point(data = g_pc1, 
#                     aes(term, estimate, colour = main2),
#                     position = position_dodge(0.5), size = 4) + ylim(-0.075, 0.11) +      
#    labs(colour = "Model terms", y = "Effect size PC1") + 
#   scale_color_manual(values = c("SNSP" = "#414487FF",
#                                   "SNWI" = "#2A788EFF",
#                                   "sst_var" = "#7AD151FF",
...
#                                   "Year" = "dark grey"))
# 
#   ggplot(g1,  aes(term, estimate)) +
#     geom_hline(yintercept= 0, alpha = 0.8) +
# geom_line() +
# geom_pointrange(data = g1, #shape = 1,
#                   aes(term, estimate, colour = main1, ymin= conf.low, ymax= conf.high, shape = res),
#                   position = position_dodge(0.5), size = 1.5) +
#   geom_point(data = g1,
#                   aes(term, estimate, colour = main2, shape = factor(res)),
...
b1 <- ggplot(g1,  aes(term, estimate, colour = res)) +
      geom_hline(yintercept= 0, alpha = 0.8) +
  geom_line() +
  geom_pointrange(data = g1,
                    aes(term, estimate, colour = res, ymin= conf.low, ymax= conf.high),
                    position = position_dodge(0.5), size = 1) #+ ylim(-0.25, 0.25) #+ ylim(-0.15, 0.25)

b2 <- ggplot(g2,  aes(term, estimate, colour = res)) +
      geom_hline(yintercept= 0, alpha = 0.8) +
  geom_line() +
  geom_pointrange(data = g2,
                    aes(term, estimate, colour = res, ymin= conf.low, ymax= conf.high),
                    position = position_dodge(0.5), size = 1) #+ ylim(-0.25, 0.25) #+ ylim(-0.15, 0.25)

b3 <- ggplot(g3,  aes(term, estimate, colour = res)) +
      geom_hline(yintercept= 0, alpha = 0.8) +
  geom_line() +
  geom_pointrange(data = g3,
                    aes(term, estimate, colour = res, ymin= conf.low, ymax= conf.high),
                    position = position_dodge(0.5), size = 1) #+ ylim(-0.25, 0.25) #+ ylim(-0.15, 0.25)
```

```{r, fig.width= 14, fig.height= 20}
b1 <- b1 + theme(axis.text.x = element_blank(), axis.title.x = element_blank())
b2 <- b2 + theme(axis.text.x = element_blank(), axis.title.x = element_blank())
b3 <- b3 + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), axis.title.x = element_blank()) 
...
b1 <- ggplot(g1,  aes(term, estimate, group = res)) +
    geom_hline(yintercept= 0, alpha = 0.8) +
            ...
            colour = NA, alpha = 0.01) +
geom_pointrange(data = g1, 
                  aes(term, estimate, colour = main1, ymin= conf.low, ymax= conf.high, shape = factor(res)),
                  position = position_dodge(0.9), size = 1.5) +
  geom_point(data = g1,
                  aes(term, estimate, colour = main2, shape = factor(res)),
             position = position_dodge(0.9), size = 4) +
  labs(colour = "Model terms", y = "Effect size PC1", group = "resolution") + 
  scale_color_manual(values = c("SNSP" = "#414487FF",
                                "SNWI" = "#2A788EFF",
                                "sst_var" = "#7AD151FF",
                                "Depth" =  "orange",
...
b2 <- ggplot(g2,  aes(term, estimate, group = res)) +
  ...
  geom_pointrange(data = g2, 
    ...
    geom_point(data = g2,
                  aes(term, estimate, colour = main2, shape = factor(res)),
                position = position_dodge(0.9), size = 4) +
  labs(colour = "Model terms", y = "Effect size PC2", group = "resolution") + 
    scale_color_manual(values = c("SNSP" = "#414487FF",
                                  "SNWI" = "#2A788EFF",
                                  "sst_var" = "#7AD151FF",
                                  "Depth" =  "orange",
...
b3 <- ggplot(g3,  aes(term, estimate, group = res)) +
  ...
  geom_pointrange(data = g3, 
    ...
    geom_point(data = g3,
                  aes(term, estimate, colour = main2, shape = factor(res)),
                position = position_dodge(0.9), size = 4) +
  labs(colour = "Model terms", y = "Effect size PC3", group = "resolution") + 
    scale_color_manual(values = c("SNSP" = "#414487FF",
                                  "SNWI" = "#2A788EFF",
                                  "sst_var" = "#7AD151FF",
                                  "Depth" =  "orange",
...
fp <- read.dbf("~/Desktop/covars/fp_ICES_2009_2020/shapefiles/total-2020.dbf")
  ...
  f <- f[f$lon <= tmp_raster@extent@xmax  & f$lon >= tmp_raster@extent@xmin & f$lat <= tmp_raster@extent@ymax  & f$lat >= tmp_raster@extent@ymin,]
...
points(-15, 55.5, cex = 5, pch  = 0, col = "black",  lwd = 2)
points(-15, 54, cex = 5*(218/250), pch  = 0, col = "orange",  lwd = 2)
points(-15, 52.5, cex = 5*(197/250), pch  = 0, col = "orange",  lwd = 2)
points(-15, 51, cex = 5*(183/250), pch  = 0, col = "orange",  lwd = 2)
points(-15, 49.5, cex = 5*(210/250), pch  = 0, col = "orange",  lwd = 2)
points(-15, 48, cex = 5*(138/250), pch  = 0, col = "orange",  lwd = 2)
points(-15, 46.5, cex = 5*(150/250), pch  = 0, col = "red",  lwd = 2)


#scalebar(250, xy=NULL, type='bar', divs=2, label = "0  20  40",adj=c(1, 1), cex = 0.5)
...
par(mfrow = c(3,2), bty = "n", yaxt="n", xaxt="n", mar = c(0.5,0,0.5,0.5))

plot(tmp_raster, ext=extent(tmp_raster),xaxs="i",yaxs="i", col = pal(50), legend=FALSE)
plot(land, col = "grey", add =T, ext=extent(tmp_raster),xaxs="i",yaxs="i", legend=FALSE)
plot(t50, ext=extent(tmp_raster),xaxs="i",yaxs="i", col = pal(50))
plot(land, col = "grey", add =T, ext=extent(tmp_raster),xaxs="i",yaxs="i", legend=FALSE)
plot(-depth, ext=extent(tmp_raster),xaxs="i",yaxs="i", col = pal(50), legend=FALSE)
plot(land, col = "grey", add =T, ext=extent(tmp_raster),xaxs="i",yaxs="i", legend=FALSE)
plot(-d50, ext=extent(tmp_raster),xaxs="i",yaxs="i", col = pal(50))
plot(land, col = "grey", add =T, ext=extent(tmp_raster),xaxs="i",yaxs="i", legend=FALSE)
plot(f, ext=extent(tmp_raster),xaxs="i",yaxs="i", col = pal(50), legend=FALSE)
plot(land, col = "grey", add =T, ext=extent(tmp_raster),xaxs="i",yaxs="i", legend=FALSE)
plot(fp50, ext=extent(tmp_raster),xaxs="i",yaxs="i", col = pal(50))
plot(land, col = "grey", add =T, ext=extent(tmp_raster),xaxs="i",yaxs="i", legend=FALSE)
...
## homa ranges
```{r}
hr <- read.csv("~/Desktop/covars/probst2021_northseahomeranges.csv")
